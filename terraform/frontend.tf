module "frontend_website" {
  ...
  # Next.js static export (output: 'export') generates a real out/404.html for
  # every unmatched route — unlike a client-routed SPA, there's no single
  # index.html that should catch everything, so errors go to the real 404 page
  # instead of silently 200-ing the homepage.
...
resource "null_resource" "build_frontend" {
  triggers = {
    # Rebuild if frontend source or static assets change
    ...
    # Force re-evaluation of exported files by including timestamp
...
resource "null_resource" "sync_frontend_to_s3" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      aws s3 sync ${path.module}/../out s3://${module.frontend_website.s3_bucket_id}/ \
        --delete \
